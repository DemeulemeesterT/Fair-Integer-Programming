...
lottery LeximaxMasterLegacy::solve(bool print) {
	...
	while (sum > 0) {
		...
		model->update();
		model->write("Generated Formulations/LeximaxMasterLegacy.lp");
...
void LeximaxMasterLegacy::addColumn(solution sol, bool print) {
	GRBColumn col;
	int counter = 0;
	for (int i = 0; i < K->I.n; i++) {
		if (K->M[i] == 1) {
...
void LeximaxMasterLegacy::getDualValues(bool print) {
	dual_C.clear();
	dual_C_Sum1.clear();
...
void LeximaxMasterLegacy::defineModelConVar(bool print) {
	env = new GRBEnv();
	model = new GRBModel(*env);
	env->set(GRB_StringParam_LogFile, "Generated Log-files/LeximaxMasterLegacy.log");
...
LeximaxMasterLegacy::LeximaxMasterLegacy(IPSolver* K_in, bool print) {
	K = new IPSolver(K_in, print);

	defineModelConVar(print);
}

LeximaxMasterLegacy::~LeximaxMasterLegacy() {
	delete[] C;
	delete K;
	delete model;
	delete env;
